...
export class ClientDetailsPageComponent {
  ...
  ngOnInit(): void {
    const clientID = this.route.snapshot.paramMap.get('id') != null ? this.route.snapshot.paramMap.get('id') : "";
    this.clientService.getClientByID(clientID == null ? " " : clientID).subscribe((res: Client) => {
  ...
  deleteClientHandler(client: Client): void {
    this.clientService.deleteClient(client.id != null ? client.id : "").subscribe(() => {
      this.backToSearchPage();
    });
  }
